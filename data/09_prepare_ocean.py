...
if __name__ == "__main__":
    ...
    start = time.time()
    dem_ds = xr.open_dataset("dem/BedMachineGreenland-v5.nc").thin({"x": thin, "y": thin})
    ...
    for gcm in ["MIROC5_RCP85"]:
        ...
        mask.rio.set_spatial_dims(x_dim="x", y_dim="y")
        mask.rio.write_crs(crs, inplace=True)

        mask.to_netcdf("ocean/mask.nc")
        ...
        deepest_index = deepest_index.rio.set_spatial_dims(x_dim="x", y_dim="y")
        deepest_index.rio.write_crs(crs, inplace=True)
        ...
        depth_indices = np.abs(depths[:, None, None] - target_depth[None, :, :]).argmin(axis=0)  # (y, x)
        ...
        print("Merging forcing, summing up")

        ds = xr.merge([forcing_3d, basin_mask]).sum(dim="basin_id").load()
