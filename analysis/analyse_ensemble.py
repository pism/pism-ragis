...
def plot_basins(observed, simulated, plot_var, filtering_var):
    ...
    for k, basin in enumerate(observed.basin):
        ...
        sim_prior = (
            simulated.sel(basin=basin, ensemble_id="RAGIS").rolling(time=13).mean()
        )
        ...
        _ = ax.fill_between(
            quantiles[0.5].time,
            quantiles[0.16][plot_var],
            quantiles[0.84][plot_var],
            alpha=0.4,
            color=sim_cmap[1],
            label=sim_prior["Ensemble"].values,
            lw=0,
        ...
        _ = ax.fill_between(
            obs["time"],
            obs[plot_var] - obs[plot_var_uncertainty],
            obs[plot_var] + obs[plot_var_uncertainty],
            color=obs_cmap[0],
            alpha=0.4,
            lw=0,
...
if __name__ == "__main__":
    ...
    with Client(cluster, asynchronous=True) as client:
        ...
        for response_var in [
            ...
            responses = to_analyze.sel(basin="GIS", ensemble_id="RAGIS")[response_var]
    ...
    all_delta_indices: xr.Dataset = xr.concat(all_delta_indices_list, dim="name")
