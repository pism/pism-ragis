...
def hello_kitty(url, chunks=None):
    print(lds)
    ...
    for k, basin in basins.iterrows():
        ...
        basin_file = result_dir / f"rignot_basin_{basin_name}_ensemble_id_{ensemble_id}_exp_id_{exp_id}_sums.nc"
        start = time.time()
        print(f"Computing {basin_name} sums and saving to {basin_file}")
...
if __name__ == "__main__":
    ...
    for exp in experiments:
        ...
        urls = url.glob(f"""*_gris_g{exp["resolution"]}m*.nc""")
        for m_url in urls:
            ds = xr.open_dataset(m_url, chunks=chunks)
            if options.temporal_range:
                ds = ds.sel(time=slice(*options.temporal_range))
            m_id_re = re.search("id_(.+?)_", ds.encoding["source"])
            ds = ds.expand_dims("exp_id")
    ...
    #     urls = url.glob(f"""*_gris_g{exp["resolution"]}m*.nc""")
    #     print("foo")
    #     with tqdm_joblib(tqdm(desc="Processing files", total=len(list(urls)))) as p:
    #         Parallel(n_jobs=options.n_jobs)(
    #             delayed(hello_kitty)(url) for url in list(urls)
    #         )
