...
#         "tendency_of_ice_mass_due_to_grounding_line_flux",
#     ]
...
#     basins_sums["ice_mass"] = (
#         basins_sums["ice_mass"]
#         - basins_sums.sel(time="1980-01-01", method="nearest")["ice_mass"]
#     )

#     gris_sums = xr.open_dataset(gris_file, chunks="auto")
#     gris_sums["ice_mass"] = (
#         gris_sums["ice_mass"]
#         - gris_sums.sel(time="1980-01-01", method="nearest")["ice_mass"]
#     )

#     domain_sums = xr.open_dataset(domain_file, chunks="auto")
#     domain_sums["ice_mass"] = (
#         domain_sums["ice_mass"]
#         - domain_sums.sel(time="1980-01-01", method="nearest")["ice_mass"]
#     )
...
#         da = da.load()
#         # s_med = scalar_sums.sel(ensemble_id=ens_id, exp_id="BAYES-MEDIAN")
#         d_med = domain_sums.sel(ensemble_id=ens_id).median(dim="exp_id")
#         g_med = gris_sums.sel(ensemble_id=ens_id).median(dim="exp_id")
...
#         nrows=3, ncols=1, sharex="col", figsize=(7, 4.2), height_ratios=[16, 9, 9]
#     )
...
#         da = da.load()
#         s_med = scalar_sums.sel(ensemble_id=ens_id, exp_id="BAYES-MEDIAN")
#         d_med = domain_sums.sel(ensemble_id=ens_id, exp_id="BAYES-MEDIAN")
#         g_med = gris_sums.sel(ensemble_id=ens_id, exp_id="BAYES-MEDIAN")
...
#         axs[1].plot(
#             g_med.time,
#             g_med.rolling({"time": 13}).mean()[sim_discharge_varname],
#             lw=1.5,
#             color=sim_colors[3],
#             label="Ice Sheet",
#         )
#         axs[1].plot(
#             d_med.time,
#             d_med.rolling({"time": 13}).mean()[sim_discharge_varname],
#             lw=1.5,
#             color=sim_colors[1],
#             label="Modeling Domain",
#         )
#         axs[2].plot(
#             g_med.time,
#             g_med.rolling({"time": 13}).mean()[sim_smb_varname],
#             lw=1.5,
#             color=sim_colors[3],
#             label="Ice Sheet",
#         )
#         axs[2].plot(
#             d_med.time,
#             d_med.rolling({"time": 13}).mean()[sim_smb_varname],
#             lw=1.5,
#             color=sim_colors[1],
#             label="Modeling Domain",
#         )
...
#     legend_obs.get_frame().set_linewidth(0.0)
#     legend_obs.get_frame().set_alpha(0.0)
#     legend_sim = axs[0].legend(handles=sim_cis, loc="lower left", title="Simulated")
#     legend_sim.get_frame().set_linewidth(0.0)
#     legend_sim.get_frame().set_alpha(0.0)
...
#     fig.subplots_adjust(
#         wspace=0, hspace=0.075, bottom=0.125, top=0.975, left=0.175, right=0.96
#     )
#     gris = sums.sel(basin="GIS")
#     for k, (ens_id, da) in enumerate(gris.groupby("ensemble_id")):
#         da = da.load()
...
#         axs[1].plot(
#             q_med["time"],
#             q_med.rolling({"time": 13}).mean()[sim_discharge_varname],
#             lw=0.75,
#             color=sim_colors[k],
#         )
...
#         axs[2].plot(
#             da.time,
#             q_med.rolling({"time": 13}).mean()[sim_smb_varname],
#             lw=0.75,
#             color=sim_colors[k],
#         )
